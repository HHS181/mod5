...
function configuration_install() {
    source <(grep SERVER= download.sh)
    source alis.conf
...
function check_variables() {
    ...
    check_variables_list "AUR" "$AUR" "aurman yay pacaur pikaur"
    ...
    if [ -n "$AUR" -o -n "$PACKAGES_AUR" ]; then
        ...
        case "$AUR" in
            ...
            "yay"|"pikaur" )
                ...
                ;;
            "pacaur" )
                arch-chroot /mnt bash -c "pacman -S expac yajl jq meson gtest gmock --noconfirm"
                arch-chroot /mnt bash -c "echo -e \"$USER_PASSWORD\n$USER_PASSWORD\n$USER_PASSWORD\n$USER_PASSWORD\n\" | su $USER_NAME -c \"cd /home/$USER_NAME && mkdir auracle && cd auracle && curl -o PKGBUILD https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=auracle-git && makepkg -si --noconfirm && cd && rm -rf auracle\""
                arch-chroot /mnt bash -c "echo -e \"$USER_PASSWORD\n$USER_PASSWORD\n$USER_PASSWORD\n$USER_PASSWORD\n\" | su $USER_NAME -c \"cd /home/$USER_NAME && mkdir $AUR && cd $AUR && curl -o PKGBUILD https://aur.archlinux.org/cgit/aur.git/plain/PKGBUILD?h=pacaur && makepkg -si --noconfirm && cd && rm -rf $AUR\""
                ;;
        esac
    ...
    if [ "$REBOOT" == "true" ]; then
        ...
        if [ "$ASCIINEMA" == "false" ]; then
            ...
            for (( i = 15; i >= 1; i-- )); do
                ...
                if [ $? -eq 0 ]; then
                    echo ""
                    echo "Restart aborted. You must do an explicit reboot (./alis-reboot.sh)."
                    echo ""
                    REBOOT="false"
                    break
                fi
            ...
            echo ""
            echo "Restart aborted. You must terminate asciinema recording and do an explicit reboot (exit, ./alis-reboot.sh)."
            echo ""
            REBOOT="false"
        fi
        ...
        if [ "$ASCIINEMA" == "false" ]; then
            echo ""
            echo "You must do an explicit reboot (./alis-reboot.sh)."
            echo ""
        else
            echo ""
            echo "You must terminate asciinema recording and do an explicit reboot (exit, ./alis-reboot.sh)."
            echo ""
        fi
    fi
}
